...
function renderNotifications() { const target = $('#registrationNotifications'); const empty = $('#registrationNoticeEmpty'); if (!target || !empty) return; empty.hidden = notifications.length > 0; target.innerHTML = notifications.map(n => `<div class="announcement-mini"><span>●</span><div><strong>${escapeHtml(n.title)}</strong><small>${escapeHtml(n.body)}</small></div></div>`).join('') }
