...
let docs = (store.get('hub-docs', defaultDocs) || []).map((doc, index) => ({ ...doc, id: doc.id ?? index + 1 })), tickets = store.get('hub-tickets', defaultTickets), employees = store.get('hub-employees', defaultEmployees), announcements = store.get('hub-announcements', defaultAnnouncements), notifications = [], history = store.get('hub-history', []);
...
function setView(id) { if (id === 'admin' && currentUser?.role !== 'admin') return; $$('.view').forEach(v => v.classList.toggle('active-view', v.id === id)); $$('.nav-item').forEach(v => v.classList.toggle('active', v.dataset.view === id)); const labels = { dashboard: ['KHÔNG GIAN LÀM VIỆC', `Chào buổi sáng, ${currentUser?.name || 'Đạt FPT'}!`], chat: ['TRỢ LÝ AI', 'Tra cứu tri thức nội bộ'], documents: ['KHO TRI THỨC', 'Tài liệu nội bộ'], employees: ['DANH BẠ NHÂN SỰ', 'Nhân sự công ty'], announcements: ['TRUYỀN THÔNG NỘỘC BỘ', 'Thông báo'], ticket: ['HỖ TRỢ NỘI BỘ', 'Yêu cầu hỗ trợ'], admin: ['QUẢN TRỊ HỆ THỐNG', 'Cài đặt và phân quyền'] }; $('#sectionLabel').textContent = labels[id][0]; $('#pageTitle').textContent = labels[id][1]; window.scrollTo(0, 0) }
...
function renderEmployees() { const q = $('#employeeSearch').value.toLowerCase(); $('#employeeList').innerHTML = employees.filter(e => Object.values(e).join(' ').toLowerCase().includes(q)).map(e => `<article class="employee-card"><div class="avatar">${e.initial}</div><h3>${escapeHtml(e.name)}</h3><p>${escapeHtml(e.role)}</p><small>${escapeHtml(e.department)} · Đang hoạt động</small>${currentUser?.role === 'admin' && e.id ? `<button class="secondary-button" data-employee-delete="${e.id}">Xóa nhân sự</button>` : ''}</article>`).join('') }
...
async function reloadState() { try { const data = await api('/api/state'); docs = (data.documents || []).map(d => ({ id: d.id, name: d.name, category: d.category || 'Chung', date: new Date(d.created_at).toLocaleDateString('vi-VN'), status: d.status })); tickets = (data.tickets || []).map(t => ({ id: t.id, title: t.title, type: t.type, priority: t.priority, date: t.date, status: t.status })); announcements = (data.announcements || []).map(a => ({ id: a.id, title: a.title, body: a.body, date: a.date })); employees = (data.employees || []).map(e => ({ id: e.id, name: e.name, role: e.role, department: e.department, initial: e.initial })); notifications = data.notifications || []; renderNotifications(); if (currentUser?.role === 'admin') await reloadUsers(); saveAll(); } catch (error) { console.warn(error.message) } }
async function reloadUsers() { const target = $('#pendingUsers'); if (!target) return; try { const data = await api('/api/admin/users'); const users = data.users || []; target.innerHTML = users.length ? users.map(u => `<div class="announcement-mini"><div><strong>${escapeHtml(u.name)} · ${escapeHtml(u.email)}</strong><small>Đăng ký: ${new Date(u.created_at).toLocaleString('vi-VN')} · Trạng thái: ${escapeHtml(u.approval_status)} · Quyền: ${escapeHtml(u.role)}</small><input data-user-position="${u.id}" value="${escapeHtml(u.position || 'Nhân viên')}" placeholder="Chức vụ"><select data-user-role="${u.id}"><option value="member" ${u.role === 'member' ? 'selected' : ''}>Thành viên</option><option value="manager" ${u.role === 'manager' ? 'selected' : ''}>Quản lý</option></select><select data-user-status="${u.id}"><option value="approved" ${u.approval_status === 'approved' ? 'selected' : ''}>Duyệt</option><option value="rejected" ${u.approval_status === 'rejected' ? 'selected' : ''}>Từ chối</option></select><button class="secondary-button" data-user-save="${u.id}">Lưu</button><button class="secondary-button" data-user-message="${u.id}">Nhắn tin</button><button class="secondary-button" data-user-delete="${u.id}">Xóa</button></div></div>`).join('') : '<p class="empty-state">Chưa có tài khoản nào.</p>'; } catch (error) { target.innerHTML = `<p class="empty-state">${escapeHtml(error.message)}</p>`; } }
function renderNotifications() { const target = $('#registrationNotifications'); const empty = $('#registrationNoticeEmpty'); if (target && empty) { const registrations = notifications.filter(n => n.title === 'Tài khoản mới đăng ký'); empty.hidden = registrations.length > 0; target.innerHTML = registrations.map(n => `<div class="announcement-mini"><span>●</span><div><strong>${escapeHtml(n.title)}</strong><small>${escapeHtml(n.body)}</small></div></div>`).join('') } const privateTarget = $('#privateNotificationList'); const privateEmpty = $('#privateNotificationEmpty'); const replyForm = $('#notificationReplyForm'); if (privateTarget && privateEmpty) { privateEmpty.hidden = notifications.length > 0; privateTarget.innerHTML = notifications.map(n => `<div class="announcement-mini"><span>✉</span><div><strong>${escapeHtml(n.title)}</strong><small>${escapeHtml(n.body)} · ${new Date(n.created_at).toLocaleString('vi-VN')}</small></div></div>`).join(''); if (replyForm) replyForm.classList.toggle('hidden', currentUser?.role === 'admin' || notifications.length === 0) } }
...
$('#addEmployee').addEventListener('click', () => { if (currentUser?.role !== 'admin') return alert('Chỉ admin cấp 1 được thêm nhân sự.'); $('#employeeForm').classList.remove('hidden'); $('#employeeName').focus(); });
$('#cancelEmployee').addEventListener('click', () => $('#employeeForm').classList.add('hidden'));
$('#employeeForm').addEventListener('submit', async e => { e.preventDefault(); const name = $('#employeeName').value.trim(), role = $('#employeeRole').value.trim(), department = $('#employeeDepartment').value.trim(); try { const data = await api('/api/employees', { method: 'POST', body: JSON.stringify({ name, department, role, initial: name.split(' ').map(x => x[0]).slice(-2).join('').toUpperCase() }) }); employees.unshift({ id: data.employee.id, name: data.employee.name, department: data.employee.department, role: data.employee.role, initial: data.employee.initial }); e.target.reset(); e.target.classList.add('hidden'); saveAll(); } catch (error) { alert(error.message) } });
$('#employeeList').addEventListener('click', async e => { const button = e.target.closest('[data-employee-delete]'); if (!button) return; if (!confirm('Xóa nhân sự này khỏi database?')) return; button.disabled = true; try { await api(`/api/employees/${button.dataset.employeeDelete}`, { method: 'DELETE' }); employees = employees.filter(employee => employee.id !== Number(button.dataset.employeeDelete)); saveAll(); } catch (error) { button.disabled = false; alert(error.message) } });
...
$('#pendingUsers')?.addEventListener('click', async e => { const messageButton = e.target.closest('[data-user-message]'); const deleteButton = e.target.closest('[data-user-delete]'); const button = e.target.closest('[data-user-save]'); const id = (messageButton || deleteButton || button)?.dataset.userMessage || (messageButton || deleteButton || button)?.dataset.userDelete || (messageButton || deleteButton || button)?.dataset.userSave; if (!id) return; try { if (messageButton) { const message = prompt('Nội dung tin nhắn riêng:'); if (!message) return; await api(`/api/admin/users/${id}/message`, { method: 'POST', body: JSON.stringify({ message }) }); alert('Đã gửi tin nhắn riêng.'); } else if (deleteButton) { if (!confirm('Xóa tài khoản này khỏi database?')) return; await api(`/api/admin/users/${id}`, { method: 'DELETE' }); await reloadUsers(); await reloadState(); } else { button.disabled = true; const role = document.querySelector(`[data-user-role="${id}"]`).value; const position = document.querySelector(`[data-user-position="${id}"]`).value.trim(); const approval_status = document.querySelector(`[data-user-status="${id}"]`).value; await api(`/api/admin/users/${id}`, { method: 'PATCH', body: JSON.stringify({ role, position, approval_status }) }); await reloadUsers(); await reloadState(); } } catch (error) { alert(error.message); if (button) button.disabled = false; } });
$('#notificationReplyForm')?.addEventListener('submit', async e => { e.preventDefault(); const message = $('#notificationReply').value.trim(); if (!message) return; try { await api('/api/notifications/reply', { method: 'POST', body: JSON.stringify({ message }) }); e.target.reset(); alert('Đã gửi trả lời admin.'); } catch (error) { alert(error.message) } });
function initials(name) { return name.split(' ').map(part => part[0]).slice(-2).join('').toUpperCase() }
function renderUser() { if (!currentUser) return; $('#profileName').textContent = currentUser.name; const profileNameLarge = $('#profileNameLarge'); if (profileNameLarge) profileNameLarge.textContent = currentUser.name; $('#profileInitial').textContent = initials(currentUser.name); const adminNav = document.querySelector('[data-view="admin"]'); if (adminNav) adminNav.hidden = currentUser.role !== 'admin'; const profileRole = document.querySelector('.profile small'); if (profileRole) profileRole.textContent = currentUser.role === 'admin' ? 'Quản trị viên cấp 1 · IT' : 'Thành viên'; setView('dashboard') }
