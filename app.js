...
async function reloadUsers() { const target = $('#pendingUsers'); if (!target) return; try { const data = await api('/api/admin/users'); const users = data.users || []; target.innerHTML = users.length ? users.map(u => `<div class="announcement-mini"><div><strong>${escapeHtml(u.name)} · ${escapeHtml(u.email)}</strong><small>Đăng ký: ${new Date(u.created_at).toLocaleString('vi-VN')} · Trạng thái: ${escapeHtml(u.approval_status)} · Quyền: ${escapeHtml(u.role)}</small><select data-user-role="${u.id}"><option value="member" ${u.role === 'member' ? 'selected' : ''}>Thành viên</option><option value="manager" ${u.role === 'manager' ? 'selected' : ''}>Quản lý</option></select><select data-user-status="${u.id}"><option value="approved" ${u.approval_status === 'approved' ? 'selected' : ''}>Duyệt</option><option value="rejected" ${u.approval_status === 'rejected' ? 'selected' : ''}>Từ chối</option></select><button class="secondary-button" data-user-save="${u.id}">Lưu</button></div></div>`).join('') : '<p class="empty-state">Chưa có tài khoản nào.</p>'; } catch (error) { target.innerHTML = `<p class="empty-state">${escapeHtml(error.message)}</p>`; } }
...
$('#pendingUsers')?.addEventListener('click', async e => { const button = e.target.closest('[data-user-save]'); if (!button) return; const id = button.dataset.userSave; const role = document.querySelector(`[data-user-role="${id}"]`).value; const approval_status = document.querySelector(`[data-user-status="${id}"]`).value; button.disabled = true; try { await api(`/api/admin/users/${id}`, { method: 'PATCH', body: JSON.stringify({ role, approval_status }) }); await reloadUsers(); } catch (error) { alert(error.message); button.disabled = false; } });
