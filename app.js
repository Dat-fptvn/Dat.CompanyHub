...
$('#addEmployee').addEventListener('click', async () => { if (currentUser?.role !== 'admin') return alert('Chỉ admin cấp 1 được thêm nhân sự.'); const name = prompt('Họ tên nhân viên:'); if (!name) return; const role = prompt('Chức vụ:', 'Nhân viên') || 'Nhân viên'; const department = prompt('Phòng ban:', 'Công nghệ') || 'Chưa xác định'; try { const data = await api('/api/employees', { method: 'POST', body: JSON.stringify({ name, department, role, initial: name.split(' ').map(x => x[0]).slice(-2).join('').toUpperCase() }) }); employees.unshift({ id: data.employee.id, name: data.employee.name, department: data.employee.department, role: data.employee.role, initial: data.employee.initial }); saveAll(); } catch (error) { alert(error.message) } });
